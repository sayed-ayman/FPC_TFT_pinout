                    ...
                    const char *sckName, const char *mosiName)
{
   attemptCount++; // Increment the attempt counter
    ...
    if (gfx->begin())
    {
        ...
        delay(2000); // Show success color for 2 seconds
    }
    else
    {
...
void setup(void)
{
    ...
    // Iterate over all possible combinations of pins, leaving one pin unused
    for (size_t exclude = 0; exclude < sizeof(pins) / sizeof(pins[0]); exclude++) // Pin to exclude
    {
